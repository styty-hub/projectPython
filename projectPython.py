...
# ZMIANA: Dodano flagę pygame.RESIZABLE
screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.RESIZABLE)
pygame.display.set_caption("Survival RPG: Skalowalne Okno")
...
ITEM_TYPES = [
    {"type": "SPEED", "color": COLOR_SPEED},
    {"type": "RANGE", "color": COLOR_RANGE},
    {"type": "ATK_SPEED", "color": COLOR_ATK_SPD},
    {"type": "DAMAGE", "color": COLOR_DMG},
    {"type": "HP_BOOST", "color": COLOR_HP},
    {"type": "WEAPON", "color": COLOR_WEAPON},
]
ITEM_WEIGHTS = [20, 20, 20, 20, 18, 2]
...
while running:
    ...
    # ZMIANA: Przycisk jest aktualizowany co klatkę, aby reagował na nowy rozmiar okna
    ...
    for event in pygame.event.get():
        ...
        # ZMIANA: Obsługa zmiany rozmiaru okna
    ...
    if state == "PLAYING":
        ...
        if player.x > WORLD_WIDTH - player.radius:
            player.x = WORLD_WIDTH - player.radius
        if player.y < player.radius:
            player.y = player.radius
        if player.y > WORLD_HEIGHT - player.radius:
        ...
        if len(items) < 3 and random.random() < 0.01:
            chosen_item = random.choices(ITEM_TYPES, weights=ITEM_WEIGHTS, k=1)[0]
            items.append(
                Item(
    ...
    # ZMIANA: Używamy max(), aby kamera nie zepsuła się przy powiększeniu okna poza granice mapy
    ...
    for x in range(0, WORLD_WIDTH + 1, 100):
        pygame.draw.line(
            ...
            (x - camera_x, WORLD_HEIGHT - camera_y),
        )
    for y in range(0, WORLD_HEIGHT + 1, 100):
        pygame.draw.line(
            ...
            (WORLD_WIDTH - camera_x, y - camera_y),
        )

    pygame.draw.rect(
        screen, GREEN, (0 - camera_x, 0 - camera_y, WORLD_WIDTH, WORLD_HEIGHT), 5
    )
    ...
    for item in items:
        pygame.draw.circle(
            screen,
            item.color,
            (int(item.x - camera_x), int(item.y - camera_y)),
            item.radius,
        )
    ...
    if state != "GAME_OVER":
        ...
        range_surface = pygame.Surface(
            (player.range * 2, player.range * 2), pygame.SRCALPHA
        )
        pygame.draw.circle(
            range_surface,
            (255, 255, 255, 10),
        ...
        screen.blit(
            range_surface, (draw_player_x - player.range, draw_player_y - player.range)
        )
    ...
    pygame.draw.rect(
        screen, GREEN, (bar_x, bar_y, bar_width * player_hp_ratio, bar_height)
    )

    hp_text_surface = font_ui.render(f"{player.hp:.1f}", True, COLOR_HP)
    screen.blit(hp_text_surface, (bar_x + bar_width + 10, bar_y - 2))
    ...
    ui_elements = [
        ...
        {
            "text": f"Weapons: {player.weapons}/3",
            "color": COLOR_WEAPON if player.weapons < 3 else RED,
        },
    ]
    ...
    # ZMIANA: Zegar dynamicznie trzyma się prawej krawędzi okna, nieważne jak je rozszerzysz
    ...
    if state == "STAGE_CLEAR":
        # Nakładki automatycznie wykorzystują nowe WIDTH i HEIGHT
        ...
        screen.blit(
            clear_text, clear_text.get_rect(center=(WIDTH / 2, HEIGHT / 2 - 100))
        )
